...
K4Grower::K4Grower(int num_worker_threads_, bool skip_final_enum_, std::ostream* log_,
    ...
    if (log_result != nullptr) {
      for (const auto& r : results) {
        ...
        *log_result << "G[" << base_graph_id
                    << "] min_theta=" << min_theta_graph.get_theta_ratio().to_string() << "\n  ";
...
std::vector<Graph> K4Grower::grow_step(int n, const std::vector<Graph>& base_graphs) {
  assert(n < Graph::N);
...
void K4Grower::enumerate_final_step(const std::vector<Graph>& base_graphs) {
  ...
  if (num_worker_threads == 0) {
    ...
    for (int i = 0; i < num_worker_threads; i++) {
      worker_threads.push_back(std::thread(&K4Grower::worker_thread_main, this, i));
    }
...
void K4Grower::worker_thread_main(int thread_id) {
...
void K4Grower::print_before_final(const std::vector<Graph> collected_graphs[MAX_VERTICES]) const {
  if (log != nullptr) {
...
void K4Grower::print_state_to_stream(
    std::ostream& os, const std::vector<Graph> collected_graphs[MAX_VERTICES]) const {
  os << "\n---------------------------------\n"
...
void K4Grower::print_config(std::ostream& os) const {
