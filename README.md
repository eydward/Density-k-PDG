...
An optimization idea (not yet implemented: if we precompute the automorphism group of each graph in `canonicals[n-1]`, before the edge set generation. Then for all vertex permutations that are automorphisms of g, we only need to try the edge sets on them once. I may try this after validating correctness of existing implementations and if the algorithm is still too slow for (4,7).
