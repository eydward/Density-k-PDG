...
Intuitively, the Contains T_k optimization works more effectively when the base graph is dense (making the generated graphs more likely to contain T_k), whereas the min_theta optmization works more effectively when the base graph is sparse (making the generated graph more likely to have a large theta_ratio value). Together, they reduce the computational complexity from 6^20 per base graph to something much more feasible. 
