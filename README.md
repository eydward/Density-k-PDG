...
* In order to build and run the code, a c++ 17 compatible compiler is required. We used `gcc (Rev8, Built by MSYS2 project) 11.2.0`, but any `gcc-11` release  should work. 
    ...
    - On Linux just run `sudo apt install build-essential`. Note that Ubuntu doesn't have gcc-11 in the default repo, follow instructions [here](https://stackoverflow.com/questions/67298443/when-gcc-11-will-appear-in-ubuntu-repositories) to install `gcc-11`.
...
* The theta value for `K=3, N=5` in the table is mathematically proved.
...
Intuitively, the Contains T_k optimization works more effectively when the base graph is dense (making the generated graphs more likely to contain T_k), whereas the min_theta optmization works more effectively when the base graph is sparse (making the generated graph more likely to have a large theta value). Together, they reduce the computational complexity from 6^20 per base graph to something much more feasible. 
